...
class Virement(db.Model):
    ...
    id = db.Column(db.String(5), primary_key=True, autoincrement=True)
...
def add_virement():
    ...
    virement = Virement(id=id, type=type, date_prod=date_prod, montant=montant, description=description)
    db.session.add(virement)
    db.session.commit()
    return jsonify({'message': 'Virement added successfully'}), 201
